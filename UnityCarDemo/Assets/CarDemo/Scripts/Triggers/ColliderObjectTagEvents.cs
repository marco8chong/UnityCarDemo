...
namespace CarDemo
{
    ...
    public class ColliderObjectTagEvents : MonoBehaviour
    {
        [SerializeField]
        private ObjectTagSO _targetObjectTagSO = null;
        ...
        private bool IsTargetObject(Collider other)
        {
            ...
            ObjectTag tag = other.GetComponent<ObjectTag>();

            if (tag)
            {
                if (tag.ObjectTagSO == _targetObjectTagSO)
                {
                    result = true;
                }
            }
